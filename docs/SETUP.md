...
### 5.4 国内网络下载 Qwen3-0.6B

`huggingface.co` 在国内不可达：

- 直连 `Connection reset by peer`
- `HF_ENDPOINT=https://hf-mirror.com` 在 `huggingface_hub>=1.x` **不再生效**：SDK 内部会硬校验官方域名 → `FileMetadataError: Distant resource does not seem to be on huggingface.co`

**解法：默认走 ModelScope（阿里源）**

- `make preflight` 默认 `--source modelscope`，调 `modelscope.snapshot_download` 把权重下到 `~/.cache/modelscope/hub/`
- ModelScope 是独立 SDK（不依赖 huggingface_hub），无域名硬校验，国内 RTT ~200ms
- Qwen 系列在 ModelScope 上的 repo id 与 HF 完全一致（`Qwen/Qwen3-0.6B`），权重 sha256 一致
- 拿到本地路径后 `from_pretrained(local_path, local_files_only=True)`，**不再触网**，第二次秒级

如果你在境外，可显式切回 HF：

```bash
uv run python scripts/preflight.py --source hf
```

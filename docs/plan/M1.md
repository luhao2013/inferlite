...
>
> **拆分**：M1 Phase 1（数值对齐）（L0+L1） → M1 Phase 2（出字）闭环（L2+L3）
>
...
| `head_dim` | 128 | 每个 attention head 的维度，是独立超参；Qwen3-0.6B 中 1024/16=64，不等于 128 |
...
**可并行**：T1 / T2 / T3 三张卡互相独立（前置都是 T0），M1·P1 实际推进时可以两线开。

---

### 3.3 Reference code 读取纪律

| 阶段 | 主参考 | 辅助参考 | 暂不看 |
| --- | --- | --- | --- |
| T1-T3 叶子算子 | `transformers` 固定版 `modeling_qwen3.py` | rasbt Qwen3 notebook | vLLM engine / llama.cpp / SGLang |
| T4 GQA Attention | `transformers::Qwen3Attention` | `vllm::Qwen3Attention` 只看 QK-norm/RoPE/GQA 组织 | TP/quant/cache kernel |
| T5-T6 拼层/整模 | `transformers::Qwen3DecoderLayer/Qwen3Model` | vLLM decoder layer 只看结构差异 | serving/runtime |
| T7 权重加载 | HF state_dict key | `vllm::Qwen3ForCausalLM.load_weights` | qkv/gate-up packing 的实现细节 |
| T8 logits 对齐 | transformers full model logits | 无 | vLLM 不作为数值 reference |
| T9-T11 出字闭环 | 自己的 minimal loop | nano-vllm / vLLM `EngineCore.step()` | SGLang/TGI/TensorRT-LLM |
...
QwenLM/Qwen3、llama.cpp、SGLang、TGI、TensorRT-LLM 统一记录在 `docs/kb/knowledge.md`；M1 只按上表读取，不泛读。

## 4. 任务卡总表（M1·P1）

> **每张卡的完整 7 字段（前置/产出/算法核心/L0测试/DoD/坑/估时）** 在 `docs/tasks/M1-archive/M1-TX-*.md` 独立文件中维护。
> 这里只列索引与状态。索引文件：[docs/tasks/README.md](../tasks/README.md)。
> **`[P]` 列**：标 ✓ 表示与同前置的其他任务可并行开工（开两个分支或两个会话同时推）。

| # | 名称 | 状态 | 前置 | [P] | 任务卡详情 | 估时 |
| --- | --- | --- | --- | --- | --- | --- |
| **T0** | **ModelConfig dataclass** | **✅** | — | — | [M1-T0-ModelConfig.md](../tasks/M1-archive/M1-T0-ModelConfig.md) | 已完成 |
| **T1** | **RMSNorm** | **✅** | T0 | ✓ | [M1-T1-RMSNorm.md](../tasks/M1-archive/M1-T1-RMSNorm.md) | 已完成 |
| **T2** | **SwiGLU MLP** | **✅** | T0 | ✓ | [M1-T2-SwiGLU.md](../tasks/M1-archive/M1-T2-SwiGLU.md) | 已完成 |
| T3 | RoPE | ✅ | T0 | ✓ | [M1-T3-RoPE.md](../tasks/M1-archive/M1-T3-RoPE.md) | 已完成 |
| T4 | GQA Attention | ✅ | T1, T3 | — | [M1-T4-GQA-Attention.md](../tasks/M1-archive/M1-T4-GQA-Attention.md) | 已完成 |
| T5 | DecoderLayer | ✅ | T1, T2, T4 | — | [M1-T5-DecoderLayer.md](../tasks/M1-archive/M1-T5-DecoderLayer.md) | 已完成 |
| T6 | Qwen3Model | ✅ | T1, T5 | — | [M1-T6-Qwen3Model.md](../tasks/M1-archive/M1-T6-Qwen3Model.md) | 已完成 |
| T7 | WeightMap + load_from_hf | ✅ | T6 | — | [M1-T7-WeightMap-load-from-hf.md](../tasks/M1-archive/M1-T7-WeightMap-load-from-hf.md) | 已完成 |
| **T8** | **L1 logits 对齐** | ✅ | T7 | — | [M1-T8-L1-logits-alignment.md](../tasks/M1-archive/M1-T8-L1-logits-alignment.md) | 已完成 |

> **并行机会**：T1 / T2 / T3 三张卡前置都是 T0，互相独立，可同时开两条线推（M1·P1 总耗时可压缩 30%）。
> **历史 T0 (包骨架 + preflight)** 已并入 M0，不再列在 M1 表内。

### 任务卡总表（M1·P2）

| # | 名称 | 状态 | 前置 | [P] | 任务卡详情 | 估时 |
| --- | --- | --- | --- | --- | --- | --- |
| T9 | LLMModel Protocol + GreedySampler | ✅ | T8 | — | [M1-T9-LLMModel-GreedySampler.md](../tasks/M1-archive/M1-T9-LLMModel-GreedySampler.md) | 已完成 |
| T10 | EngineCore.step() 三段式 | ✅ | T9 | — | [M1-T10-EngineCore-step.md](../tasks/M1-archive/M1-T10-EngineCore-step.md) | 已完成 |
| T11 | CLI + L2 e2e | ✅ | T10 | — | [M1-T11-CLI-L2-e2e.md](../tasks/M1-archive/M1-T11-CLI-L2-e2e.md) | 已完成 |
| T12-pre | Last-token logits optimization | ✅ | T11 | — | [M1-T12-pre-last-token-logits.md](../tasks/M1-archive/M1-T12-pre-last-token-logits.md) | 已完成 |
| T12 | Real Qwen3-0.6B smoke test | ✅ | T12-pre | — | [M1-T12-smoke-test.md](../tasks/M1-archive/M1-T12-smoke-test.md) | 已完成 |

---

## 5. 模型加载策略（T7 提前讲清楚）

T7 是 M1·P1 最容易卡住的一张卡，权重映射做错整模型就崩。这里把"权重哪来 → 怎么读 → 怎么映射 → 共享权重怎么处理"一次说清。
...
T7 需要额外看 vLLM 的 Qwen3 loading 逻辑，但**只看 weight loading，不泛读 engine/scheduler**。

参考卡片：[`knowledge.md` → `vLLM — Qwen3 weight loading`](../kb/knowledge.md#vllm-qwen3-weight-loading)。

vLLM 里最值得借鉴的三点：

1. `Qwen3ForCausalLM.load_weights(weights)`：模型边界暴露加载入口。
2. `skip_prefixes=["lm_head."] if tie_word_embeddings`：tie 时跳过独立 lm_head。
3. `packed_modules_mapping`：工业版会把 `q/k/v`、`gate/up` 合并；inferlite M1 不合并，只理解它为什么存在。

inferlite T7 的取舍：

| vLLM | inferlite M1 |
| --- | --- |
| `AutoWeightsLoader` 自动映射 | 显式 `WeightMap`，可读性优先 |
| `qkv_proj` packed | 保持 `q_proj/k_proj/v_proj` 拆开 |
| `gate_up_proj` packed | 保持 `gate_proj/up_proj` 拆开 |
| tensor parallel / quant / PP | 全部不做 |
| 跳过 tied `lm_head` | 必须借鉴 |

### 5.5 tie_word_embeddings 怎么处理
...
```
M1·P1-Day 1   T0 ModelConfig ✅ + T1 RMSNorm ✅ + T2 SwiGLU ✅ + T3 RoPE（2h）
M1·P1-Day 2   T4 GQA Attention（3h，最难一张）
M1·P1-Day 3   T5 DecoderLayer（1h） + T6 Qwen3Model（2h）
M1·P1-Day 4   T7 WeightMap + load_from_hf（3h） + T8 L1 logits 对齐（2h）→ M1·P1 收官

M1·P2-Day 1   T9 Sampler + Protocol（1h） + T10 EngineCore（3h）
M1·P2-Day 2   T11 CLI + L2 e2e（2h）+ 文章草稿 → 打 v0.1 tag
```
...
> 概念（形状/超参/upcast/tie_embed 等）见 `docs/kb/knowledge.md` → Concepts。
> 库 API（transformers/pytest/uv 等）见 `docs/kb/knowledge.md` → Libraries。
> 论文（RMSNorm/SwiGLU/RoPE/GQA）见 `docs/kb/knowledge.md` → Papers。

## 12. 参考项目与论文

> 见 [docs/kb/knowledge.md](../kb/knowledge.md) — 三梯队开源项目（nano-vllm / LLMs-from-scratch / transformers / vllm / ...）+ 论文清单 + 阶段-资料对照表。
> M1·P1 阶段重点：**rasbt/LLMs-from-scratch Qwen3 notebook** + **transformers/modeling_qwen3.py**，写每个模块前各读一次对应章节。
